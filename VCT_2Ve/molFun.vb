...
Module molFun
    ...
    Sub Add_SNode(ByVal X As Decimal, ByVal Y As Decimal)
        ...
        End Using
    End Sub
    Function Add_Bar_Dot_YL1_Tren(X1 As Decimal, Y1 As Decimal, X2 As Decimal, Y2 As Decimal, A As Decimal, Fi1 As Decimal, Aa1 As Decimal, Fi2 As Decimal, Aa2 As Decimal, Is_bot As Boolean, ThepCham_L1_LopTren As Boolean, Optional a_bardot As Integer = 15) As ArrayList
    ...
    End Function
    Function Add_Bar_Dot_YL1_Duoi(X1 As Decimal, Y1 As Decimal, X2 As Decimal, Y2 As Decimal, A As Decimal, Fi1 As Decimal, Aa1 As Decimal, Fi2 As Decimal, Aa2 As Decimal, Is_bot As Boolean, ThepCham_L1_LopDuoi As Boolean, Optional a_bardot As Integer = 15) As ArrayList
        Dim L As Line = New Line(New Point3d(X1, Y1, 0), New Point3d(X2, Y2, 0))
        Dim Length As Decimal = L.Length - 104
        Dim N As Decimal = Length \ A ' ví dụ N = 5
        Dim Center_SNode As Decimal = (Length - A * N) / 2
        ...
        For i As Integer = 0 To N
            ...
            If i = 1 AndAlso ThepCham_L1_LopDuoi = True Then
                'addline(l.getpointatdist(52 + i * a + center_snode).x, l.getpointatdist(52 + i * a + center_snode).y)
                Dim P1_TagThep As New Point3d(L.GetPointAtDist(52 + i * A + Center_SNode).X, L.GetPointAtDist(52 + i * A + Center_SNode).Y, 0)
                Dim P2_TagThep As New Point3d(L.GetPointAtDist(52 + i * A + Center_SNode).X, L.GetPointAtDist(52 + i * A + Center_SNode).Y - 150, 0)
                Dim P3_TagThep As New Point3d(L.GetPointAtDist(52 + i * A + Center_SNode).X + 400, L.GetPointAtDist(52 + i * A + Center_SNode).Y - 150, 0)
